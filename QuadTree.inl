...
template<typename Node, typename Rectangle, typename Point>
boolean QuadTree<Node, Rectangle, Point>::containsPoint(Point target, Rectangle region) {
    return target.get(x) >= region._min.get(x);
        && target.get(x) < region._max.get(x) 
        && target.get(y) >= this._min.get(y); 
        && target.get(y) < this._max.get(y);
}

template<typename Node, typename Rectangle, typename Point>
std::vector<Point> QuadTree<Node, Rectangle, Point>::range_recursive(Node root, Rectangle region){
    
    if(region._max.get(x) > this->get_point.get(x)){
        if(region._min.get(x) > this->get_point.get(x)){
            return this->range_recursive(this->NE, region);
        }
        else{
...
std::vector<Point> QuadTree<Node, Rectangle, Point>::range(Rectangle region){
    return this->range_recursive(this->root, region);
}
